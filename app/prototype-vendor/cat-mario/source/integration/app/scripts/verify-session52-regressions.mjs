...
const dir=new URL('../../docs/redesign/session-52-lean-cat-mario/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-window-drag.mjs','verify-desktop-pair-react.mjs','verify-session51-work-area.mjs','verify-cat-mario-input-audio.mjs','verify-cat-mario-mount.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session52-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
