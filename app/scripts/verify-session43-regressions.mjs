...
const dir=new URL('../../docs/redesign/session-43-drawer-audio/',import.meta.url),results=[];
const checks=['verify-paper-toss-entry.mjs', 'verify-drawer-audio-react-order.mjs', 'verify-drawer-motion.mjs', 'verify-drawer-state.mjs', 'verify-drawer-audio.mjs', 'verify-drawer-audio-wiring.mjs', 'verify-runtime-dispatch.mjs', 'verify-runtime-composition.mjs', 'verify-runtime-diagnostics.mjs', 'verify-device-fan-activation.mjs', 'verify-fan-continuity.mjs', 'verify-fan-motion.mjs', 'verify-fan-assembly-input.mjs', 'verify-fan-state.mjs', 'verify-fan-audio.mjs', 'verify-fan-audio-wiring.mjs', 'verify-trackpad-audio.mjs', 'verify-trackpad-wiring.mjs', 'verify-paper-audio.mjs', 'verify-paper-motion-audio.mjs', 'verify-paper-lifecycle.mjs', 'verify-paper-toss-physics.mjs', 'verify-paper-toss-scene.mjs', 'verify-paper-toss-wiring.mjs', 'verify-paper-toss-audio.mjs', 'verify-screen-projection.mjs', 'verify-phone-escape.mjs', 'verify-phone-return-camera.mjs', 'verify-desktop-marquee.mjs', 'verify-window-drag.mjs', 'verify-entry-lifecycle.mjs', 'verify-recorded-walkthrough.mjs', 'verify-second-walkthrough.mjs'];
for(const name of checks.filter(name=>!process.env.SESSION43_CHECKS||process.env.SESSION43_CHECKS.split(',').includes(name))){const original=new URL(name,import.meta.url);let script=original,temp,s=fs.readFileSync(original,'utf8');
 ...
 s=s.replaceAll("new URL('../../docs/redesign/session-30b-cat-mario/","new URL('../../docs/redesign/session-43-drawer-audio/").replaceAll("path.join(root,'../docs/redesign/session-30b-cat-mario')","path.join(root,'../docs/redesign/session-43-drawer-audio')");
 s=s.replaceAll('session-32-device-fan-activation/guard-gap.json','session-43-drawer-audio/guard-gap.json');
 s=s.replaceAll("../../docs/redesign/session-11-laptop-lighting/","../../docs/redesign/session-43-drawer-audio/");
 if(name==='verify-ground-shadows.mjs')s=s.replaceAll("../../docs/redesign/session-09-ground-shadows/","../../docs/redesign/session-43-drawer-audio/");
 if(name==='session16-verify-lamp-lighting.mjs')s=s.replaceAll("../../docs/redesign/session-16-palette-bin/","../../docs/redesign/session-43-drawer-audio/");
 if(name==='verify-entry.mjs')s=s.replace('docs/redesign/session-13-entrance/','../docs/redesign/session-43-drawer-audio/');
 if(name==='verify-brighter-lamp.mjs')s=s.replace("new URL('lighting-checks.json',dir)","new URL('../../docs/redesign/session-43-drawer-audio/lighting-checks.json',import.meta.url)");
 ...
 if(name==='verify-fan-motion.mjs')s=s.replace("new URL('../../docs/redesign/session-17-desk-fan/motion-checks.json'","new URL('../../docs/redesign/session-43-drawer-audio/motion-checks.json'");
 if(name.startsWith('verify-runtime-'))s=s.replaceAll('session-36-runtime/','session-43-drawer-audio/');
 if(name==='verify-fan-assembly-input.mjs')s=s.replace("new URL('assembly-input.json',dir)","new URL('../../docs/redesign/session-43-drawer-audio/assembly-input.json',import.meta.url)");
 // Keep the historical scene fixture read location; only result writes move.
 s=s.replaceAll('../../docs/redesign/session-43-drawer-audio/scene-fixture.mjs','../../docs/redesign/session-29b-paper-toss/scene-fixture.mjs');
 s=s.replaceAll('session-38-drawer-audio','session-43-drawer-audio');
 if(name==='verify-paper-toss-entry.mjs')s=s.replaceAll('session-42-paper-toss-entry','session-43-drawer-audio');
 if(s!==fs.readFileSync(original,'utf8')){temp=new URL('.session43-'+name,import.meta.url);fs.writeFileSync(temp,s);script=temp;}
...
const combined=process.env.SESSION43_CHECKS?JSON.parse(fs.readFileSync(new URL('focused-checks.json',dir),'utf8')).map(old=>results.find(r=>r.check===old.check)||old):results;
