...
for(const name of checks){const original=new URL(name,import.meta.url);let s=fs.readFileSync(original,'utf8');
 ...
 s=s.replaceAll('session-27-desktop-marquee/selection-checks.json','session-41-phone-to-laptop/selection-checks.json');
 if(name==='verify-runtime-dispatch.mjs')s=s.replaceAll('session-36-runtime/','session-41-phone-to-laptop/');
 if(name==='verify-cat-mario-mount.mjs'||name==='verify-cat-mario-composed-mount.mjs')s=s.replaceAll('session-40-cat-mario-launch','session-41-phone-to-laptop');
 if(name==='verify-phone-edges.mjs'||name==='verify-printer-occlusion.mjs')s=s.replaceAll('session-08-desk-camera','session-41-phone-to-laptop');
 // Historical fixture reads remain unchanged.
 const temp=new URL('.session41-'+name,import.meta.url);fs.writeFileSync(temp,s);
