...
import {Rig,Scene,DeskFan,Ball,harness} from '../../docs/redesign/session-29b-paper-toss/scene-fixture.mjs';
import {createPaperToss} from '../.vite/toss-physics.mjs';
...
check('Real Rig eligibility waits for drawer/page transitions; stable neutral camera and saved manual/passive state',()=>{
 ...
 f.props.deskInput.current.manual=true;f.props.orbit.current={yaw:.3,pitch:.1};f.run(3);const before=f.camera.position.clone();assert.ok(game.enter());f.run(4);assert.equal(game.phase,'ready');assert.ok(f.camera.position.distanceTo(deskPose(f.size).position)<1e-9);const stable=f.camera.matrixWorld.clone();f.run(3);assert.deepEqual(f.camera.matrixWorld.elements,stable.elements);assert.equal(f.props.deskPaused,true);
