...
const meshes=[];printer.traverse(o=>{if(o.isMesh)meshes.push(o)});// Session 39 adds four actual support feet; all original casing/tray meshes remain.
assert.equal(meshes.length,26);
const feet=meshes.slice(-4);for(const foot of feet){const b=new T.Box3().setFromObject(foot);assert.ok(Math.abs(b.min.y)<.00002);assert.ok(b.max.y<.15);}
...
const evidence=path.join(root,'../docs/redesign/session-08-desk-camera');
