...
const dir=new URL('../../docs/redesign/session-59-fan-entry-cable/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-session55-geometry.mjs','verify-props-resources.mjs','verify-fan-motion.mjs','verify-fan-audio.mjs','verify-session58-entry.mjs','verify-session56-shadows.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session59-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
