...
const dir=new URL('../../docs/redesign/session-32-device-fan-activation/',import.meta.url),results=[];
for(const name of ['runtime','input-audio','wiring','navigation']){
 const original=new URL(`verify-cat-mario-${name}.mjs`,import.meta.url),temp=new URL(`.session32-game-${name}.mjs`,import.meta.url);let source=fs.readFileSync(original,'utf8');
 source=source.replace("fs.writeFileSync(path.join(evidence,'runtime-results.json')","fs.writeFileSync(path.join(root,'../docs/redesign/session-32-device-fan-activation/runtime-results.json')").replace("../../docs/redesign/session-30b-cat-mario/input-audio-results.json","../../docs/redesign/session-32-device-fan-activation/input-audio-results.json").replace("../docs/redesign/session-30b-cat-mario/wiring-results.json","../docs/redesign/session-32-device-fan-activation/wiring-results.json");fs.writeFileSync(temp,source);
