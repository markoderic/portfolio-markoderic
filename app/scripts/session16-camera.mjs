...
const dir=new URL('../../docs/redesign/session-16-palette-bin/camera/',import.meta.url);
const meshes=JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-07-chair-refinement/after-render.json',import.meta.url))).meshes.filter(m=>!['desk','plant','lamp','printer','bin'].includes(m.root)).concat(...['session-15-drawers/open-desk.json','session-12-plant/after-plant.json','session-16-palette-bin/rose-props.json'].map(p=>JSON.parse(fs.readFileSync(new URL('../../docs/redesign/'+p,import.meta.url))).meshes));
...
fs.writeFileSync(new URL('framing.json',dir),JSON.stringify({frames,maxExtent,geometrySource:'Session 07 furniture/M3, Session 10 lamp, Session 12 plant',offline:true},null,2)+'\n');
