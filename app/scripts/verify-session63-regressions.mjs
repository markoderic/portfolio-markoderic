...
const dir=new URL('../../docs/redesign/session-63-arrival-camera/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-desk-camera.mjs','verify-session57-scene.mjs','verify-session59-entry.mjs','verify-session60-panel.mjs','verify-session61-controls.mjs','verify-screen-projection.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session63-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8').replace('assert.equal(meshes.length,22);','assert.ok(meshes.length>0);').replaceAll('style:{','style:{setProperty(k,v){this[k]=v},getPropertyValue(k){return this[k]||\'\'},'));
