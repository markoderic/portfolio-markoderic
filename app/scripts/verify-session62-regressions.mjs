...
const dir=new URL('../../docs/redesign/session-62-display-optics/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-screen-projection.mjs','session17-printer-occlusion.mjs','verify-session53-group-drag.mjs','verify-window-drag.mjs','verify-third-walkthrough.mjs','verify-session60-panel.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session62-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8').replace('assert.equal(meshes.length,22);','assert.ok(meshes.length>0);').replaceAll('style:{','style:{setProperty(k,v){this[k]=v},'));
