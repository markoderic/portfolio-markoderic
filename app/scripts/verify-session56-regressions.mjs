...
const dir=new URL('../../docs/redesign/session-56-shadow-continuity/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-ground-shadows.mjs','verify-session39-shadows.mjs','verify-drawer-motion.mjs','verify-drawer-audio-react-order.mjs','verify-drawer-audio-delayed-wiring.mjs','verify-production-props.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session56-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
