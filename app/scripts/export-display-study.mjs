...
const app=fileURLToPath(new URL('../',import.meta.url)),dir=path.join(app,'../docs/redesign/session-22-display-depth');
...
for(const theme of ['light','dark']){
 ...
 for(const kind of ['before','a','after']){save(theme+'-'+kind,svg(W,H,body+face(kind)))}
...
const frames=[];for(const theme of ['light','dark'])for(const night of [false,true])for(const [view,eye,target]of [['overview',desk.position,desk.look],['focused',focused,look],['angled',angled.position,angled.look],['approach',desk.position.clone().lerp(focused,.88),desk.look.clone().lerp(look,.88)],['printer',new T.Vector3(-7.4,4.4,7.4),new T.Vector3(-3.65,.55,.55)]])for(const kind of ['before','after'])frames.push({name:`${view}-${theme}-${night?'night':'day'}-${kind}`,view,eye:eye.toArray(),look:target.toArray(),width:1440,height:900,theme,night,texture:`${theme}-${kind}.png`});
// Alternative at ordinary focused framing; no runtime selector.
for(const theme of ['light','dark'])frames.push({name:`focused-${theme}-day-a`,view:'focused',eye:focused.toArray(),look:look.toArray(),width:1440,height:900,theme,night:false,texture:`${theme}-a.png`});
fs.writeFileSync(path.join(dir,'frames.json'),JSON.stringify(frames,null,2));fs.writeFileSync(path.join(dir,'fixtures.json'),JSON.stringify({fixtures,aperture:LAPTOP,method:'Actual SSR content/icons; manually composed SVG chrome/text layout; approximated CSS glass samples; not browser rendering.'},null,2));console.log('Exported 6 evidence-only texture studies, actual SSR and 42 matched frame specifications. No runtime textures added.');
