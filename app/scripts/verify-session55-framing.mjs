...
const dir=new URL('../../docs/redesign/session-55-prop-placement/',import.meta.url),data=JSON.parse(fs.readFileSync(new URL('after-scene.json',dir)));
for(const m of data.meshes){for(const k of Object.keys(m))if(!['positions','indices','name','root','side'].includes(k))delete m[k];}
const sizes=[[1440,900],[800,500]].map(([width,height])=>({width,height}));
...
check('current furniture, fully open drawers and full fan envelope fit neutral/passive/hover/manual matrix',()=>{
 ...
 const passive=Array.from({length:9},(_,i)=>deskAngles(i*5)),manual=Array.from({length:3},(_,x)=>Array.from({length:3},(_,y)=>({yaw:-.48+x*.48,pitch:-.16+y*.38/2}))).flat();
