...
const dir=new URL('../../docs/redesign/session-23-gray-printer-fuller-bin/',import.meta.url),bin=materialize(harness(Bin,{}).tree),papers=[];
...
assert.ok(wall>.002);const fill=(top-BIN_FLOOR)/(.76-BIN_FLOOR);assert.ok(fill>.64&&fill<.70);
...
let peelPaperGap=Infinity;for(const paper of papers){const h=contactHeight(triangles(peel.geometry),triangles(paper.geometry,paper.position.toArray())).height;if(Number.isFinite(h)){assert.ok(h<0,'paper penetrates peel');peelPaperGap=Math.min(peelPaperGap,-h);}}
