...
const dir=new URL('../../docs/redesign/session-51-dock-work-area/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-window-drag.mjs','verify-desktop-pair-react.mjs','verify-session50-game.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session51-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
