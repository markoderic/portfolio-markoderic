...
const dir=new URL('../../docs/redesign/session-64-scene-atmosphere/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-session63-entry.mjs','verify-session61-controls.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session64-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8').replace('assert.equal(meshes.length,22);','assert.ok(meshes.length>0);').replaceAll('style:{','style:{setProperty(k,v){this[k]=v},getPropertyValue(k){return this[k]||\'\'},'));
