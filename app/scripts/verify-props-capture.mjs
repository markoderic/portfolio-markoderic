import {mountProps} from './support/production-props-fixture.mjs';
...
const refs=Object.fromEntries([['desk',Desk],['chair',Chair],['plant',Plant],['bin',Bin]].map(([id,C])=>[id,{current:materialize(harness(C,{}).tree)}])),clock=await mountClock(),fan=mountFan(),scene=new T.Scene();scene.add(...Object.values(refs).map(r=>r.current),clock,fan.root,mountProps().root);
...
const result={pass:true,frames:900,clockFixtureUpdates:writes,captures,signatureUnchanged:true,note:'Same caster refs as Scene: clock/fan/production props are siblings outside cached floor casters; mocked renderer capture, actual geometry/signatures. Not WebGL performance evidence.'};
fs.writeFileSync(new URL('../../docs/redesign/session-19-production-props/capture-check.json',import.meta.url),JSON.stringify(result,null,2)+'\n');console.log('PASS clock updates and running fan preserve initial + single drawer-settled capture');
