...
const dir=new URL('../../docs/redesign/session-13-entrance/',import.meta.url);
const meshes=JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-07-chair-refinement/after-render.json',import.meta.url))).meshes.filter(m=>!['plant','lamp'].includes(m.root)).concat(...['session-12-plant/after-plant.json','session-10-lamp-lighting/after-lamp.json'].map(p=>JSON.parse(fs.readFileSync(new URL('../../docs/redesign/'+p,import.meta.url))).meshes));
