...
const dir=new URL('../../docs/redesign/session-61-paper-toss/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-session58-entry.mjs','verify-session57-scene.mjs','verify-paper-toss-audio.mjs','verify-paper-lifecycle.mjs','verify-paper-trajectory.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session61-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
