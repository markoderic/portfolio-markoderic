...
const dir=new URL('../../docs/redesign/session-09-ground-shadows/',import.meta.url);
const meshes=JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-07-chair-refinement/after-render.json',import.meta.url))).meshes.filter(m=>['desk','chair','plant','bin'].includes(m.root));
...
const maps=[];
for(const config of [{name:'before',resolution:1024,width:24,depth:24,center:[0,-6.315,0],far:3.4,quadratic:false,steps:[24*2.7/256,24*2.7*.4/256],day:.46,night:.5},...GROUND_SHADOW.layers.map(c=>({...c,width:GROUND_SHADOW.width,depth:GROUND_SHADOW.depth,center:GROUND_SHADOW.center,quadratic:true,steps:[c.blurStep]}))]){
...
fs.writeFileSync(new URL('offline-maps.json',dir),JSON.stringify({maps,note:'CPU approximation of depth/blur. Before uses nearest-depth union, not old GPU draw ordering. Not actual Three compositing.'},null,2)+'\n');
