...
import Items from './src/prototype/DesktopItems.jsx';
...
check('marquee Escape restores selection and stays in laptop; held Escape stays local and fresh Escape returns to desk',()=>{
 const s=setup('#laptop'),mac=harness(MacDesktop,desktop(s.h).props),props=find(mac,n=>n.type===Items).props;
 const host={current:{offsetWidth:1040,offsetHeight:650,getBoundingClientRect:()=>({left:0,top:0,width:1040,height:650})}};
 const items=harness(Items,{...props,enabled:true,size:{width:1040,height:650},host});
 const icons=()=>[...nodes(items.tree)].filter(n=>n.type===Icon),node={...captureNode(),querySelectorAll:()=>icons().map(n=>({dataset:{launcher:n.props.id},offsetLeft:n.props.position.x,offsetTop:n.props.position.y,offsetWidth:80,offsetHeight:82}))};
 icons()[0].props.select();items.render();
 const event={target:node,currentTarget:node,pointerId:5,pointerType:'mouse',button:0,clientX:400,clientY:300,preventDefault(){},stopPropagation(){}};
 items.tree.props.onPointerDown(event);items.tree.props.onPointerMove({...event,clientX:1030,clientY:620});items.render();assert.ok(find(items,n=>n.props?.className==='desktop-marquee'));
 press(s.h,[items.tree.props.onKeyDown,mac.tree.props.onKeyDown]);items.render();assert.equal(view(s.h),'laptop');assert.deepEqual(icons().filter(n=>n.props.selected).map(n=>n.props.id),['finder']);assert.equal(node.held,false);
 press(s.h,[items.tree.props.onKeyDown,mac.tree.props.onKeyDown],{repeat:true});assert.equal(view(s.h),'laptop');press(s.h,[items.tree.props.onKeyDown,mac.tree.props.onKeyDown]);assert.equal(view(s.h),'desk');
});
...
`;
const output = path.join(root, ".vite/marquee-escape-test.mjs");
await build({
