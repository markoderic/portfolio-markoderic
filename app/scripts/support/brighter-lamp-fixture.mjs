...
export async function lampFixture(before=false) {
 const output=path.join(root,'.vite/brighter-lamp-fixture-'+(before?'before':'after')+'.mjs');
 await build({stdin:{contents:"export {Lamp,Lighting,Desk,Printer,Chair,Plant,Bin} from './src/prototype/StudioProps.jsx';export {default as ProductionProps} from './src/prototype/ProductionProps.jsx';export {default as DeskFan} from './src/prototype/DeskFan.jsx';export {default as DeskClock} from './src/prototype/DeskClock.jsx';export {harness,nodes} from './scripts/support/hook-harness.js';",resolveDir:root,loader:'jsx'},bundle:true,platform:'node',format:'esm',packages:'external',jsx:'transform',outfile:output,plugins:[{name:'lamp-offline',setup(b){
  ...
  if(before)b.onLoad({filter:/\/(StudioProps.jsx|lampLighting.js)$/},a=>({contents:fs.readFileSync(path.join(root,'../docs/redesign/session-34-brighter-lamp/before',path.basename(a.path)),'utf8'),loader:a.path.endsWith('.jsx')?'jsx':'js',resolveDir:path.dirname(a.path)}));
 }}]});
 ...
 function materialize(node) {
  ...
  if(constructors[node.type]||node.type===undefined||node.type==='primitive'){
   ...
   for(const k of ['name','userData','raycast','visible','castShadow','receiveShadow','intensity','angle','penumbra','distance','decay','target'])if(p[k]!==undefined)o[k]=p[k];
  ...
  if(['meshStandardMaterial','meshBasicMaterial'].includes(node.type)){const {children,ref,...params}=p,m=new T[node.type==='meshStandardMaterial'?'MeshStandardMaterial':'MeshBasicMaterial'](params);if(ref)ref.current=m;return m;}
  return null;
 }
