...
const output=path.join(root,'.vite/session57-scene-fixture.mjs');
await build({stdin:{contents:`export {Printer,Desk,Bin,Chair,Plant,Lamp} from './src/prototype/StudioProps.jsx';export {Rig,default as Scene} from './src/prototype/Scene.jsx';export {default as DeskFan} from './src/prototype/DeskFan.jsx';export {default as Ball} from './src/prototype/PaperTossBall.jsx';export {harness} from './scripts/support/hook-harness.js';export {createPaperToss} from './src/prototype/paperToss.js';export {tossPose} from './src/prototype/paperTossCamera.js';`,resolveDir:root,loader:'jsx'},bundle:true,platform:'node',format:'esm',jsx:'transform',outfile:output,plugins:[{name:'offline-scene',setup(b){b.onResolve({filter:/^framer-motion$/},()=>({path:path.join(root,'node_modules/framer-motion/dist/es/index.mjs'),external:true}));b.onResolve({filter:/^three$/},()=>({path:path.join(root,'node_modules/three/build/three.module.js'),external:true}));
 ...
 b.onLoad({filter:/.*/,namespace:'fixture'},a=>({resolveDir:root,contents:a.path==='react'?`import * as h from './scripts/support/hook-harness.js';export * from './scripts/support/hook-harness.js';export const forwardRef=fn=>props=>fn(props,null);export default {...h,forwardRef};`:a.path==='fiber'?`export const Canvas=()=>null;export const useFrame=(fn,priority=0)=>{fn.priority=priority;globalThis.__printerFrames.push(fn);};`:`export {RoundedBox} from './node_modules/@react-three/drei/core/RoundedBox.js';export const useGLTF=()=>{throw Error('Not used in geometry fixture')};export const useTexture=()=>{};export const Environment=()=>null;export const Lightformer=()=>null;export const ContactShadows=()=>null;`}));
...
globalThis.__printerFrames=[];const {Printer,Desk,Bin,Chair,Plant,Lamp,Rig,Scene,DeskFan,Ball,harness,createPaperToss,tossPose}=await import(pathToFileURL(output).href+'?v='+Date.now());
...
function materialize(node){
 ...
 const geometries={sphereGeometry:T.SphereGeometry,planeGeometry:T.PlaneGeometry,torusGeometry:T.TorusGeometry,extrudeGeometry:T.ExtrudeGeometry,boxGeometry:T.BoxGeometry,cylinderGeometry:T.CylinderGeometry,circleGeometry:T.CircleGeometry,latheGeometry:T.LatheGeometry};
...
export {Printer,Desk,Bin,Chair,Plant,Lamp,Rig,Scene,DeskFan,Ball,harness,materialize,createPaperToss,tossPose};
