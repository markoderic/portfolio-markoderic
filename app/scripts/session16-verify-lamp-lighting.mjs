...
const dir=new URL('../../docs/redesign/session-16-palette-bin/',import.meta.url),api=await lampFixture(),old=await lampFixture(true);
...
check('one source lies just outside the shade; cone rays clear the lip before reaching nearby objects',()=>{
 ...
 for(const polar of [0,.2,.5,LAMP_LIGHT.angle])for(let i=0;i<24;i++){const az=i*Math.PI/12,d=v([Math.sin(polar)*Math.cos(az),-Math.cos(polar),Math.sin(polar)*Math.sin(az)]).transformDirection(a.head.matrixWorld);ray.set(pos(a.spot),d);assert.equal(ray.intersectObjects(meshes,false).filter(h=>h.distance<.35).length,0);}
...
check('emission and illumination share one transition during rapid toggles, reduced motion and initial mounts',()=>{
 for(const initialNight of [false,true]){const h=api.mount({night:initialNight}),p=parts(h);assert.equal(p.spot.intensity/LAMP_LIGHT.intensity,p.emitter.material.emissiveIntensity/LAMP_LIGHT.emission);assert.equal(p.spot.intensity,initialNight?LAMP_LIGHT.intensity:0);
 for(let i=0;i<40;i++){const night=i%3!==0;h.render({night});h.frame(i%2?.016:.04);const value=p.spot.intensity/LAMP_LIGHT.intensity;assert.ok(value>=0&&value<=1);assert.ok(Math.abs(value-p.emitter.material.emissiveIntensity/3)<1e-12);}
 for(const night of [true,false,true,false]){h.render({night,reduced:true});h.frame();assert.equal(p.spot.intensity,night?LAMP_LIGHT.intensity:0);assert.equal(p.emitter.material.emissiveIntensity,night?3:0);}
 }
});
check('shade/support geometry and day lighting/exposure are preserved with Session 34 night endpoints; exterior never becomes emissive',()=>{
 function shell(h){const list=[];h.object.traverse(o=>{if(o.isMesh&&o.geometry.type!=='CircleGeometry')list.push({positions:[...o.geometry.attributes.position.array],matrix:o.matrixWorld.toArray(),color:o.geometry.type==='LatheGeometry'?null:o.material.color.toArray()});});return list;}
 ...
 for(const night of [false,true]){const h=api.mount({night,reduced:true}),oldH=old.mount({night,reduced:true});h.frame();oldH.frame();const levels=x=>{const list=[];x.scene.traverse(o=>{if(o.isLight&&!o.isSpotLight)list.push([o.type,o.intensity,o.color.toArray(),pos(o).toArray()])});return list;};if(!night)assert.deepEqual(levels(h),levels(oldH));else {const expected=[.38,.42,.72];levels(h).forEach((v,i)=>{assert.ok(Math.abs(v[1]-expected[i])<1e-10);assert.deepEqual(v.slice(2),levels(oldH)[i].slice(2));});}assert.equal(h.gl.toneMappingExposure,oldH.gl.toneMappingExposure);h.object.traverse(o=>{if(o.isMesh&&o.geometry.type!=='CircleGeometry')assert.equal(o.material.emissive.getHex(),0);});}
