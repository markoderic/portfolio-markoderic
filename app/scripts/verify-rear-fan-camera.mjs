...
const dir=new URL('../../docs/redesign/session-24-fan-placement-activation/camera/',import.meta.url);
const meshes=JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-07-chair-refinement/after-render.json',import.meta.url))).meshes.filter(m=>!['desk','plant','lamp','printer','bin'].includes(m.root)).concat(...['session-15-drawers/open-desk.json','session-12-plant/after-plant.json','session-23-gray-printer-fuller-bin/after-props.json','session-24-fan-placement-activation/center-fan.json','session-18-vintage-clock/day-clock.json','session-19-production-props/selected-props.json'].map(p=>JSON.parse(fs.readFileSync(new URL('../../docs/redesign/'+p,import.meta.url))).meshes));
