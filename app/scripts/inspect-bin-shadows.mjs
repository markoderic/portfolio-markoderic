...
const root=new URL('../../docs/redesign/session-16-palette-bin/',import.meta.url);const tag=process.argv.find(a=>a.startsWith('--state='))?.slice(8)||'closed';if(!['closed','open','top','middle','bottom'].includes(tag))throw Error('Unknown drawer state');const dir=new URL(tag+'/',root);fs.mkdirSync(dir,{recursive:true});
const meshes=JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-07-chair-refinement/after-render.json',import.meta.url))).meshes.filter(m=>['chair'].includes(m.root)).concat(JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-15-drawers/'+tag+'-desk.json',import.meta.url))).meshes,JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-12-plant/after-plant.json',import.meta.url))).meshes);
meshes.push(...JSON.parse(fs.readFileSync(new URL('rose-props.json',root))).meshes.filter(m=>m.root==='bin'));
