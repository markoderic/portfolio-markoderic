...
const dir=new URL('../../docs/redesign/session-50-game-input/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-session50-game.mjs','verify-cat-mario-runtime.mjs','verify-cat-mario-input-audio.mjs','verify-cat-mario-wiring.mjs','verify-cat-mario-navigation.mjs','verify-cat-mario-mount.mjs','verify-desktop-pair-react.mjs','verify-window-drag.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session50-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
