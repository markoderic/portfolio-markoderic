...
const dir=new URL('../../docs/redesign/session-19-production-props/camera/',import.meta.url);
const meshes=JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-07-chair-refinement/after-render.json',import.meta.url))).meshes.filter(m=>!['desk','plant','lamp','printer','bin'].includes(m.root)).concat(...['session-15-drawers/open-desk.json','session-12-plant/after-plant.json','session-16-palette-bin/rose-props.json','session-17-desk-fan/center-fan.json','session-18-vintage-clock/day-clock.json','session-19-production-props/selected-props.json'].map(p=>JSON.parse(fs.readFileSync(new URL('../../docs/redesign/'+p,import.meta.url))).meshes));
...
check('new props leave clock, lamp/fan and drawer control witness rays open across manual/passive camera extremes',()=>{
 const propMeshes=[...roots.camera.children,...roots.microphone.children],ray=new T.Raycaster();
 const witnesses=[new T.Vector3(3.15,.13,-1.05),...roots.fan.children.filter(m=>['fan-base','fan-switch'].includes(m.name)).map(m=>{m.geometry.computeBoundingBox();return m.geometry.boundingBox.getCenter(new T.Vector3())}),...roots.desk.children.filter(m=>m.name==='drawer-handle').map(m=>{m.geometry.computeBoundingBox();return m.geometry.boundingBox.getCenter(new T.Vector3())})];
 const clock=roots.clock.getObjectByName('clock-segments');clock.geometry.computeBoundingBox();for(const fraction of [0,.25,.5,.75,1]){const b=clock.geometry.boundingBox;witnesses.push(new T.Vector3(b.min.x+(b.max.x-b.min.x)*fraction,(b.min.y+b.max.y)/2,(b.min.z+b.max.z)/2));}
 let samples=0;for(const size of [{width:1440,height:900},{width:800,height:900},{width:2560,height:1080}])for(const angles of [...Array.from({length:17},(_,i)=>deskAngles(i*2.5)),...[-.48,.48].flatMap(yaw=>[-.16,.22].map(pitch=>({yaw,pitch})))]){const pose=deskPose(size,angles);for(const point of witnesses){ray.set(pose.position,point.clone().sub(pose.position).normalize());ray.far=point.distanceTo(pose.position);const hit=ray.intersectObjects(propMeshes,false)[0];assert.ok(!hit,'New prop masks a control/clock witness '+JSON.stringify({point,angles,name:hit?.object.name}));samples++;}}
 fs.writeFileSync(new URL('control-rays.json',dir),JSON.stringify({samples,note:'New prop triangle rays only; prior fan/lamp overlap still pending. Finite witnesses, not all pixels.'},null,2));
});
...
const propMeshes=[...roots.camera.children,...roots.microphone.children];propMeshes.forEach(m=>m.geometry.computeBoundingBox());const clockHit=new T.Vector3(),ray=new T.Ray(),propRay=new T.Raycaster();let samples=0;
check('production props clear sampled live laptop and phone sightlines during camera/device transitions',()=>{
  ...
  for(const view of ['laptop','desk','phone','desk','printer','paper','desk']){
   f.render({view});for(let tick=0;tick<100;tick++){f.frame(1/30);for(const [screen,width,height]of [[f.props.laptop.current,LAPTOP.width,LAPTOP.height],[f.props.phone.current,PHONE.width,PHONE.height]]){
    screen.updateWorldMatrix(true,false);for(let x=0;x<=4;x++)for(let y=0;y<=4;y++){
     const point=new T.Vector3((x/4-.5)*width,(y/4-.5)*height,0).applyMatrix4(screen.matrixWorld),distance=point.distanceTo(f.camera.position);ray.set(f.camera.position,point.clone().sub(f.camera.position).normalize());const candidates=propMeshes.filter(m=>ray.intersectsBox(m.geometry.boundingBox));propRay.set(ray.origin,ray.direction);propRay.far=distance-.001;const at=propRay.intersectObjects(candidates,false)[0];assert.ok(!at,'prop before live screen '+JSON.stringify({view,tick,size,angles,screen:screen===f.props.laptop.current?'laptop':'phone',point:point.toArray(),camera:f.camera.position.toArray(),hit:at?{name:at.object.name,point:at.point}:null}));samples++;
    }
   }}
  }
 }
});
fs.writeFileSync(new URL('props-projection.json',dir),JSON.stringify({samples,method:'Actual Rig camera/phone poses; rays through 5x5 grids of both measured displays against separate prop bounds followed by actual triangles. Sampled views/time/grid, not browser compositing proof.'},null,2)+'\n');
