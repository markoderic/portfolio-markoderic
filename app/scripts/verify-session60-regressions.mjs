...
const dir=new URL('../../docs/redesign/session-60-calendar-reveal/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-recorded-walkthrough.mjs','verify-second-walkthrough.mjs','verify-desktop-pair-react.mjs','verify-session51-work-area.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session60-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
