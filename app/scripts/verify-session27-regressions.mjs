...
for(const name of checks){const original=new URL(name,import.meta.url);let script=original,temp,s=fs.readFileSync(original,'utf8');
 s=s.replaceAll("new URL('../../docs/redesign/session-25-fan-audio/", "new URL('../../docs/redesign/session-27-desktop-marquee/").replaceAll("new URL('../../docs/redesign/session-26-paper-timing-audio/", "new URL('../../docs/redesign/session-27-desktop-marquee/");
 if(s!==fs.readFileSync(original,'utf8')){temp=new URL('.session27-'+name,import.meta.url);fs.writeFileSync(temp,s);script=temp;}
