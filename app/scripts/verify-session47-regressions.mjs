...
const dir=new URL('../../docs/redesign/session-47-wood-camera/',import.meta.url),results=[];
const checks=['verify-session37-geometry.mjs','verify-production-props.mjs','verify-props-resources.mjs','verify-session39-shadows.mjs','verify-moving-drawer-shadows.mjs','verify-props-drawer-rays.mjs','verify-session37-framing.mjs','verify-phone-edges.mjs','verify-paper-trajectory.mjs','verify-drawer-audio-react-order.mjs','verify-desktop-pair-react.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session47-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
