...
const dir=new URL('../../docs/redesign/session-16-palette-bin/',import.meta.url),root=fileURLToPath(new URL('../',import.meta.url));
...
check('actual component bakes once, interpolates black-only day/night without recapture, restores context, cleans up',()=>{
 ...
 globalThis.shadowFrame({gl},1/60);assert.equal(group.current.visible,true);assert.equal(gl.renders.length,6);const receivers=[...nodes(h.tree)].filter(n=>n.type==='mesh');for(const n of receivers)assert.equal(n.props.raycast.toString().replace(/\s/g,''),ignoreShadowRay.toString().replace(/\s/g,''));
