...
const dir=new URL('../../docs/redesign/session-55-prop-placement/',import.meta.url),results=[];
const checks=process.argv.length>2?process.argv.slice(2):['verify-production-props.mjs','verify-props-resources.mjs','verify-props-drawer-rays.mjs','verify-paper-trajectory.mjs','verify-session49-printer.mjs'];
for(const name of checks){const output=new URL(name.replace('.mjs','')+'/',dir);fs.mkdirSync(output,{recursive:true});
 ...
 const temp=new URL('.session55-'+name,import.meta.url);fs.writeFileSync(temp,prefix+fs.readFileSync(new URL(name,import.meta.url),'utf8'));
