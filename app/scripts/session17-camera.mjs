...
const dir=new URL('../../docs/redesign/session-17-desk-fan/camera/',import.meta.url);
const meshes=JSON.parse(fs.readFileSync(new URL('../../docs/redesign/session-07-chair-refinement/after-render.json',import.meta.url))).meshes.filter(m=>!['desk','plant','lamp','printer','bin'].includes(m.root)).concat(...['session-15-drawers/open-desk.json','session-12-plant/after-plant.json','session-16-palette-bin/rose-props.json','session-17-desk-fan/center-fan.json'].map(p=>JSON.parse(fs.readFileSync(new URL('../../docs/redesign/'+p,import.meta.url))).meshes));
...
const checkResults=[];function check(name,fn){if(process.argv.includes('--projection-only')&&!name.startsWith('fan moving'))return;fn();checkResults.push(name);console.log('PASS '+name)}
